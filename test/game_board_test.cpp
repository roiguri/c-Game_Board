...
TEST_F(GameBoardTest, Initialize_EmptyBoardLines) {
    ...
    EXPECT_FALSE(board.initialize(emptyLines));
}
...
TEST_F(GameBoardTest, Initialize_ValidBoardLines) {
    ...
    EXPECT_TRUE(board.initialize(boardLines));
...
TEST_F(GameBoardTest, Initialize_IncompleteBoardLines) {
    std::vector<std::string> boardLines = {
        "###",  // Shorter than board width
        "#1 ",  // Shorter than board width
        "#"     // Shorter than board width
    };
    
    // Should initialize without error, remaining cells are empty
    EXPECT_TRUE(board.initialize(boardLines));
    
    // Check specific cells
    EXPECT_EQ(board.getCellType(0, 0), GameBoard::CellType::Wall);
    EXPECT_EQ(board.getCellType(1, 1), GameBoard::CellType::Tank1);
    EXPECT_EQ(board.getCellType(0, 2), GameBoard::CellType::Wall);
    
    // Check that cells outside the provided data are empty
...
TEST_F(GameBoardTest, ToString_PopulatedBoard) {
    ...
    smallBoard.initialize(boardLines);
    std::string expected = "# #\n1@2\n###\n";
    EXPECT_EQ(smallBoard.toString(), expected);
}