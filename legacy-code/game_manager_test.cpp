...
protected:
    ...
    // Helper to access m_tankControllers
    std::vector<GameManager::TankWithAlgorithm>& getTankControllersMutable(GameManager& manager) {
...
// Mock TankAlgorithm that always returns a fixed action
class FixedActionTankAlgorithm : public TankAlgorithm {
public:
    ActionRequest actionToReturn;
    FixedActionTankAlgorithm(ActionRequest action) : actionToReturn(action) {}
    ActionRequest getAction() override { return actionToReturn; }
    void updateBattleInfo(BattleInfo&) override {}
};

// Factory that returns FixedActionTankAlgorithm
class FixedActionTankAlgorithmFactory : public TankAlgorithmFactory {
public:
    ActionRequest actionToReturn;
    FixedActionTankAlgorithmFactory(ActionRequest action) : actionToReturn(action) {}
    std::unique_ptr<TankAlgorithm> create(int, int) const override {
        return std::make_unique<FixedActionTankAlgorithm>(actionToReturn);
    }
};
...
TEST_F(GameManagerTest, TankControllers_NextAction_PopulatedCorrectly) {
    // Board with 3 tanks
    std::vector<std::pair<int, Point>> tankPositions = {
        {1, Point(0,0)}, {2, Point(1,0)}, {1, Point(2,0)}
    };
    GameManager manager(std::make_unique<PlayerFactoryImpl>(), std::make_unique<FixedActionTankAlgorithmFactory>(ActionRequest::MoveForward));
    // Directly set m_tanks for the test
    auto& tanks = getTanksMutable(manager);
    tanks.clear();
    for (const auto& [playerId, pos] : tankPositions) {
    ...
    std::cout << "Creating tank algorithms" << std::endl;
    callCreateTankAlgorithms(manager);
    // All tanks should have nextAction == DoNothing before processStep
    auto& controllers = getTankControllersMutable(manager);
    for (const auto& controller : controllers) {
        EXPECT_EQ(controller.nextAction, ActionRequest::DoNothing);
    }
    std::cout << "Processing step" << std::endl;
    // After processStep, all alive tanks should have MoveForward
    testProcessStep(manager);
    std::cout << "Checking next action" << std::endl;
    for (const auto& controller : controllers) {
        if (!controller.tank.isDestroyed()) {
            EXPECT_EQ(controller.nextAction, ActionRequest::MoveForward);
        } else {
            EXPECT_EQ(controller.nextAction, ActionRequest::DoNothing);
        }
    }
    std::cout << "Destroying tank" << std::endl;
    // Destroy one tank and check nextAction is DoNothing after another processStep
    controllers[1].tank.destroy();
    testProcessStep(manager);
    EXPECT_EQ(controllers[1].nextAction, ActionRequest::DoNothing);
}
